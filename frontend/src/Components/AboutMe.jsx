...
function AboutMe() {
    ...
    const description = Object.keys(textDescriptions).map((v) => (
        <div key={v.toLowerCase()}>
            ...
            {textDescriptions[v].map((v) => (
                <p key={v.split(' ').slice(0, 3).join('-')}>{v}</p>
            ))}
        </div>
    ))
                ...
                <Row>
                    ...
                    <Col>
                        <Image
                            variant="bottom"
                            src={HeadshotImage}
                            className="align-self-end about-me-image"
                            alt="Malachi Crain headshot"
                        />
                    </Col>
                </Row>
            </div>
