...
const data = [
    ...
    },
    {
        ...
        buttonHref:
            'https://www.linkedin.com/posts/malachi-crain_today-marked-the-final-day-of-my-software-activity-7225259641297907712-4Ni3'
    },
    {
    ...
    },
    {
        ...
        buttonHref:
            'https://www.linkedin.com/posts/malachi-crain_over-the-past-weekend-i-had-the-incredible-activity-7171611901406224384-jhgV'
    },
    {
    ...
    },
    {
        ...
        buttonHref:
            'https://www.linkedin.com/posts/malachi-crain_its-hard-to-believe-that-its-already-the-activity-7095795278238113792-fcZs'
    },
    {
    ...
    },
    {
        ...
        buttonHref:
            'https://as.ua.edu/alumni-giving/leadership-board/outstanding-sophomore-and-junior-awards'
    }
]
...
const renderRows = (data, columns) =>
    data.reduce(createDataReducer(columns), []).map((row, index) => (
        <div
            className={`row-${index + 1} ${columns === 3 ? 'd-lg-flex d-md-none d-sm-flex' : 'd-none d-lg-none d-md-flex d-sm-none'}`}
            key={index}
        >
            <Row className="mt-4">
                {row.map((item, i) => (
...
const AchievementsList = ({ partialDisplay = false }) => {
    const displayedData = partialDisplay ? data.slice(0, 3) : data
    return (
        <div
            className={`item-list${partialDisplay ? ' partial-item-list' : ''}`}
        >
            {renderRows(displayedData, 3)}
            {renderRows(displayedData, 2)}
            {partialDisplay && (
                <Button
                    href="/achievements"
...
AchievementsList.propTypes = { partialDisplay: bool }
